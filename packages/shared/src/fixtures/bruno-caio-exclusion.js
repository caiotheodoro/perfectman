...
export function buildBrunoCaioExclusionScenario() {
    const simulationId = "sim_exclusion";
    const channelId = "ch_geral";
    const simulation = makeSimulation(simulationId, "Bruno-Caio Exclusion", ["goulart", "bruno", "caio"], [channelId]);
    ...
    const caioState = makeAgentState("caio", simulationId, "caio", { valence: 0.3, arousal: 0.5 }, {}, new Map([
    ...
    const brunoState = makeAgentState("bruno", simulationId, "bruno", { valence: -0.1, arousal: 0.4 }, {
        socialAnxiety: 0.3,
        fearOfExclusion: 0.2,
    ...
    const e1 = makeCommittedEvent(simulationId, channelId, "goulart", "message_sent", { content: "alguém ai ainda?" }, 1);
    const e2 = makeCommittedEvent(simulationId, channelId, "bruno", "reply_sent", { content: "to aqui sim", replyToId: e1.id }, 1);
    const e3 = makeCommittedEvent(simulationId, channelId, "caio", "reply_sent", { content: "opa goulart, tudo bom?", replyToId: e1.id }, 2);
    return {
        simulation,
        channels: [channel],
        memberships,
...
//# sourceMappingURL=bruno-caio-exclusion.js.map