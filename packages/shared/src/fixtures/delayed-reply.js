...
import { makeSimulation, makePublicChannel, membershipFor, makeAgentState, makeCommittedEvent, } from "./helpers.js";
export function buildDelayedReplyScenario() {
    const simulationId = "sim_delayed_reply";
    const channelId = "ch_geral";
    const simulation = makeSimulation(simulationId, "Delayed Reply", ["leo", "caio", "goulart"], [channelId]);
    ...
    const leoState = makeAgentState("leo", simulationId, "leo", { valence: -0.1, arousal: 0.35 }, { socialAnxiety: 0.2, fearOfExclusion: 0.15 });
    ...
    const leoMsg = makeCommittedEvent(simulationId, channelId, "leo", "message_sent", { content: "e aí galera, o que estão fazendo?" }, 1);
...
//# sourceMappingURL=delayed-reply.js.map