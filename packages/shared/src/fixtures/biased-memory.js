...
import { makeSimulation, makePublicChannel, membershipFor, makeAgentState, makeRelationalState, makeCommittedEvent, makeMemory, } from "./helpers.js";
export function buildBiasedMemoryScenario() {
    const simulationId = "sim_biased_memory";
    const channelId = "ch_geral";
    const simulation = makeSimulation(simulationId, "Biased Memory", ["goulart", "caio"], [channelId]);
    ...
    const goulartNegative = makeAgentState("goulart", simulationId, "goulart", { valence: -0.6, arousal: 0.4 }, { resentment: 0.3, suspicion: 0.4 }, new Map([
    ...
    const memories = [
        makeMemory("mem_pos_1", "goulart", simulationId, "episodic", ["caio"], "caio me ajudou quando eu precisei", "positive", 0.85),
        makeMemory("mem_pos_2", "goulart", simulationId, "episodic", ["caio"], "caio falou bem de mim na frente de todo mundo", "positive", 0.80),
        makeMemory("mem_neg_1", "goulart", simulationId, "emotional_residue", ["caio"], "caio me ignorou numa conversa importante", "negative", 0.75),
        makeMemory("mem_neg_2", "goulart", simulationId, "social_theory", ["caio"], "caio sempre parece condescendente comigo", "negative", 0.70),
        makeMemory("mem_neu_1", "goulart", simulationId, "social_theory", ["caio"], "caio costuma demorar pra responder", "neutral", 0.65),
    ];
    ...
    const e1 = makeCommittedEvent(simulationId, channelId, "caio", "message_sent", { content: "hey goulart, tudo bem?" }, 2);
    return {
        simulation,
        channels: [channel],
        memberships,
...
//# sourceMappingURL=biased-memory.js.map