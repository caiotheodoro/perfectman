...
export function buildSimulationFixture() {
    ...
    const simulation = makeSimulation(simulationId, "Perfectman Fixture", agentIds, [channelId]);
    const channel = makePublicChannel(channelId, simulationId, "#geral", agentIds);
    const memberships = membershipFor(channelId, agentIds);
    const agentStates = ALL_PERSONAS.map(persona => makeAgentState(persona.id, simulationId, persona.id, {
...
//# sourceMappingURL=simulation-fixture.js.map