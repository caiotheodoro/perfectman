...
export function buildPrivateChannelMotiveScenario() {
    const simulationId = "sim_private_motive";
    const channelId = "ch_geral";
    const simulation = makeSimulation(simulationId, "Private Channel Motive", ["mariana", "caio"], [channelId]);
    ...
    const marianaState = makeAgentState("mariana", simulationId, "mariana", { valence: 0.1, arousal: 0.6, energy: 0.7 }, {
        affection: 0.8,
    ...
    const e1 = makeCommittedEvent(simulationId, channelId, "caio", "message_sent", { content: "alguém tem planos pro fim de semana?" }, 1);
    return {
        simulation,
        channels: [channel],
        memberships,
...
//# sourceMappingURL=private-channel-motive.js.map