...
Build persona cognition layer. Receives `AgentRuntimeInput` (assembled by dev2), combines Dev3's engine-calibration `PersonaConfig` with Dev1-owned `PersonaPromptProfile` and `LlmConfig`, creates the persona prompt using Dev3's pre-translated emotional state from `PerceptionPacket`, calls either a local Qwen3-8B runtime or FreeLLMAPI through one OpenAI-compatible provider interface, validates structured JSON output against dev3's intent schema, tracks token budget, and returns the `ActionIntent`.

Keep the first implementation simple: no standalone LLM router is required. Provider choice comes from the Dev1 `LlmConfig` resolved for the agent, not from Dev3's `PersonaConfig`.
...
AgentRuntimeInput (built by dev2 from dev3 EngineStepResult)
  ...
  → LlmProvider: mock | openai-compatible
       - ollama: native Ollama /api/chat (local Qwen3 runs)
       - openai-compatible: any OpenAI-compatible endpoint (hosted DeepSeek, FreeLLMAPI proxy)
...
- `TranslatedEmotionalState` via `PerceptionPacket.translatedEmotionalState` — generated by dev3 engine code

**Consume from dev2 (event runtime):**
- Event runtime scheduler calls `AgentRuntime.generateIntent(input, context)` passing assembled `AgentRuntimeInput` plus `pulseIndex`/`now` context
...
packages/server/src/agent/
  ...
  prompt-builder.ts             # builds 8-section prompt from PerceptionPacket.translatedEmotionalState
...
- ~~`emotional-language.ts`~~ → consume `PerceptionPacket.translatedEmotionalState` from dev3 instead of translating raw scores
...
PersonaPromptProfile
  owner: dev1
  purpose: compact compiled LLM identity, voice, style examples, relationship prose, language/slang; not the full source-material store
...
Persona source material may later live in committed Markdown docs using real names and sanitized/paraphrased examples by default. Keep that source layer separate from runtime: self-perception, peer perception, and observed chat evidence should be manually summarized into the compact `PersonaPromptProfile` until repeated work justifies a compiler.
...
type LlmConfig = {
  ...
  timeoutMs: number;
  retryCount: number;
  extraBody?: Record<string, unknown>; // runtime-specific controls such as Qwen thinking disablement
};
```
...
type AgentRuntimeOutput = {
  ...
  llmUsage: LlmUsage | null;        // null when budget/fallback path made no provider call
  latencyMs: number;
...
4. **Subjective emotional state**: natural language only via `PerceptionPacket.translatedEmotionalState` — **no raw scores**
...
Dev3 owns `translateEmotionalState()` and stores its output on `PerceptionPacket.translatedEmotionalState`. Dev1 consumes the translated strings; it does not recompute emotional language from raw scores.
...
```typescript
type LlmProvider = {
  generateIntent(input: AgentRuntimeInput, context: AgentRuntimeContext, prompt: BuiltPrompt): Promise<LlmProviderResult>;
};
```
...
- Import `ActionIntent`, `AgentRuntimeInput`, `LlmUsage`, and `OperatorEvent` from `@perfectman/shared`
- Define `AgentRuntimeContext`, `BuiltPrompt`, `LlmProvider`, `LlmProviderResult`, `AgentRuntimeOutput`
...
### M3: PersonaPromptProfile & PersonaLoader
- Define Dev1-owned `PersonaPromptProfile` as the compact runtime prompt artifact, not the full biography/testimonial store.
- Provide initial Goulart and Bruno profiles plus safe fallback behavior.
- Resolve `LlmConfig` with mock defaults unless explicit runtime overrides are supplied.
- Do not duplicate, mutate, or replace Dev3 `PersonaConfig`.
...
- Read `input.perceptionPacket.translatedEmotionalState` for Sections 4 and 5.
...
- Track calls and token usage per simulation and per agent.
- Return controlled no-op/delay fallback when exhausted; never fake `LlmUsage` for blocked calls.

### M7: OpenAI-Compatible Provider
...
- Select the provider directly from the Dev1-resolved `LlmConfig.providerType`.
- No standalone `LlmRouter` for MVP; add one only if provider selection becomes more complex than runtime config lookup.

### M8: Runtime Orchestration & Robust Fallbacks
- Orchestrate `AgentRuntime.generateIntent(input, context)`: pre-check budget → resolve `PersonaPromptProfile` + `LlmConfig` → build prompt → create provider from `LlmConfig` → run provider → parse/repair/validate → record usage → return.
...
### M9: Exports & Integration Tests
- Export Dev1 agent/LLM surfaces from `packages/server/src/index.ts`.
- Keep provider HTTP tests mocked; do not require network in CI.
