...
**Persistence — IMPLEMENTED (2026-08-26, issue #97, [ADR-0012](../adr/0012-goal-registry-persistence.md)).** Sqlite-backed simulations persist the LLM self-verdict junction — the one registry shape replay cannot rebuild — into `goal_self_verdicts` at every review end, restore it onto the replayed registry before the first review of a restart, and resume mid-run state through the supported factory (`attachExisting` on `buildConfiguredSimulation`). Replay stays the source of truth; the DB is a write-through cache in both persistence modes, and memory-mode runs are byte-identical (the in-memory Map + replay path is unchanged).
